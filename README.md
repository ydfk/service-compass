...
通知通道负责“发到哪里”和“对哪些服务生效”；每个监控可单独设置是否通知以及使用哪些通知通道。
通知事件类型默认包含离线、恢复和警告；通知冷却时间在「设置」中统一配置。

Synology Chat 建议优先使用 Incoming Webhook，它会发送到 Webhook 绑定的频道。Chatbot 模式只适合发送给 Bot 可见的数字 `user_id`；如果要发频道，请不要使用 Chatbot Token。

## 本地开发

需要 Rust stable、Node.js 26+，以及 pnpm 11+。
...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `DATABASE_URL` | `sqlite:data/service-compass.db` | SQLite 数据库地址 |
| `SERVICECOMPASS_BIND` | `127.0.0.1:3000` | 后端监听地址 |
| `SERVICECOMPASS_STATIC_DIR` | `frontend/dist` | 前端静态文件目录 |
| `SERVICECOMPASS_LOG_DIR` | `data/logs` | 系统日志文件目录 |
| `SERVICECOMPASS_LOG_RETENTION_DAYS` | `30` | 系统日志文件保留天数 |
