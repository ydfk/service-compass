...
import { Bell, BrandDocker, Dashboard, FileText, Logout, Settings, Stack2 } from '@vicons/tabler'
...
<template>
  <NLayout has-sider class="admin-shell">
    <NLayoutSider bordered :width="240" :collapsed-width="68" :collapsed="mobile" collapse-mode="width" :show-trigger="mobile ? false : 'bar'" class="sider">
      ...
      <NButton class="logout" quaternary @click="logout"><template #icon><NIcon :component="Logout" /></template>退出登录</NButton>
    </NLayoutSider>
...
.sider { position: fixed; z-index: 5; height: 100vh; background: #0b121f; }
.admin-brand { display: flex; height: 5.5rem; align-items: center; gap: 0.7rem; padding: 0 1.2rem; overflow: hidden; color: #e8edf7; text-decoration: none; white-space: nowrap; }
...
.admin-brand small { color: #52627a; font-size: 0.56rem; letter-spacing: 0.13em; }
.logout { position: absolute; bottom: 1.2rem; left: 1rem; }
.admin-content { margin-left: 240px; min-height: 100vh; padding: 2.5rem; background: #080d17; }
